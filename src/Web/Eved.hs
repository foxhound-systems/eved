...
-- to unwrap the api
noContext :: I.Eved api m => (() -> api a) -> api a
noContext f = f ()
...
(.<|>) :: (I.Eved api m) => (ctx -> api a) -> (ctx -> api b) -> (ctx -> api (a I.:<|> b))
(.<|>) =
    liftA2 (I..<|>)

-- |Add a Literal string to the path of the api
lit :: (I.Eved api m) => Text -> (ctx -> api a) -> (ctx -> api a)
lit l = fmap (I.lit l)

-- |Add a url capture with a given name and UrlElement decoder/encoder
capture :: (I.Eved api m) => Text -> (ctx -> UE.UrlElement a) -> (ctx -> api b) -> (ctx -> api (a -> b))
...
reqBody :: (I.Eved api m) => NonEmpty (ctx -> CT.ContentType a) -> (ctx -> api b) -> (ctx -> api (a -> b))
...
queryParam :: (I.Eved api m) => Text -> (ctx -> QP.QueryParam a) -> (ctx -> api b) -> (ctx -> api (a -> b))
...
queryParams :: (I.Eved api m) => Text -> (ctx -> QP.QueryParam a) -> (ctx -> api b) -> (ctx -> api ([a] -> b))
...
verb :: (I.Eved api m) => StdMethod -> Status -> NonEmpty (ctx -> CT.ContentType a) -> (ctx -> api (m a))
verb m s ctyps = fmap (I.verb m s) (sequenceA ctyps)

get, post, put, patch, delete :: (I.Eved api m) => NonEmpty (ctx -> CT.ContentType a) -> (ctx -> api (m a))
...
(.</>) :: (I.Eved api m) =>  ((ctx -> api a) -> (ctx -> api b)) -> (ctx -> api a) -> (ctx -> api b)
(.</>) = ($)
infixr 5 .</>
